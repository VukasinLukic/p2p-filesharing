...
export default function StatusHeader() {
  ...
  // Keying the poll on apiBase restarts it against the new peer the moment settings change.
  const { data: status } = usePolling(api.status, 1000, [apiBase])
        ...
        <div className="status-dot-group">
          ...
          <div>
            ...
            <div className="status-meta">
              ...
              <code>{status?.tcpPort ?? '—'}</code> &middot; api <code>{hostLabel(apiBase)}</code>
            </div>
          </div>
          <button
            type="button"
